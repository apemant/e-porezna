...
public class repPDVDisk15 extends repDisk {
  JAXBContext context;
  ...
  private void generate() throws Exception {
    ...
    sobrazacpdv.setVerzijaSheme("9.0");
  }
...
/*    if (data.containsKey(Generator.SASTAVIO_TEL)) sastavio.setTelefon((String)data.get(Generator.SASTAVIO_TEL));
  ...
  private STijelo getTijelo() {
    ...
    tijelo.setPodatak810(data.get("Pdv810"));
    tijelo.setPodatak811(data.get("Pdv811"));
    tijelo.setPodatak812(data.get("Pdv812"));
    tijelo.setPodatak813(data.get("Pdv813"));
    tijelo.setPodatak814(data.get("Pdv814"));
    tijelo.setPodatak815(data.get("Pdv815"));
    tijelo.setPodatak820(data.get("Pdv820"));
    tijelo.setPodatak830(data.get("Pdv830"));
    tijelo.setPodatak831(addSIsporukaN(tijelo.getPodatak831(), data.get("Pdv831o"), data.get("Pdv831p")));
    tijelo.setPodatak832(addSIsporukaN(tijelo.getPodatak831(), data.get("Pdv832o"), data.get("Pdv832p")));
    tijelo.setPodatak833(addSIsporukaN(tijelo.getPodatak831(), data.get("Pdv832o"), data.get("Pdv833p")));
    tijelo.setPodatak840(data.get("Pdv840"));
    tijelo.setPodatak850(data.get("Pdv850"));
    tijelo.setPodatak860(data.get("Pdv860"));
    tijelo.setPodatak870(raSaldaKonti.isNaplata());
...
Pod600  VI. UKUPNO RAZLIKA: ZA UPLATU/ZA POVRAT
  ...
  private SIsporukaNekretnina addSIsporukaN(SIsporukaNekretnina isp, BigDecimal vri, BigDecimal broj) {
    if (isp == null) {
      isp = factory.createSIsporukaNekretnina();
      isp.setVrijednost(Aus.zero2);
      isp.setBroj(0);
    }
    vri = (vri == null)?Aus.zero2:vri;
    isp.setVrijednost(isp.getVrijednost().add(vri));
    isp.setBroj(isp.getBroj() + (broj == null ? 0 : broj.intValue()));
    return isp;
  }
  ...
  private hr.porezna_uprava.e_porezna.sheme.metapodaci.v2_0.SPDVmetapodaci getMeta() throws Exception {
    return MetaGenerator.generateSPDVmeta(raUser.getInstance().getImeUsera(), System.currentTimeMillis()+"", 
        "Prijava poreza na dodanu vrijednost", "ObrazacPDV-v9-0");
  }


  private void initialize() throws Exception {
    context = JAXBContext.newInstance("hr.porezna_uprava.e_porezna.sheme.zahtjevi.obrazacpdv.v9_0");
