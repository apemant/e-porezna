...
import hr.porezna_uprava.e_porezna.sheme.zahtjevi.obrazacjoppd.v1_1.*;
...
public class repJOPPDDisk extends repDisk {
  ...
  public repJOPPDDisk() {
    try {
      ...
      this.setPrint("JOPPD_e-porezna.xml");
    }
    catch (Exception ex) {
      ex.printStackTrace();
    }
  ...
  private void initialize() throws Exception {
    context = JAXBContext.newInstance("hr.porezna_uprava.e_porezna.sheme.zahtjevi.obrazacjoppd.v1_1");
  ...
  private void generate() throws Exception {
    ...
    sobrazacjoppd.setVerzijaSheme("1.1");
  }
  ...
  private SStranaA getStranaA() throws Exception {
    ...
    System.out.println("EPorezna podnositelj email: " + data.get(Generator.PODNOSITELJ_EMAIL));
    podnositelj.setEmail((String)data.get(Generator.PODNOSITELJ_EMAIL));
    strA.setPodnositeljIzvjesca(podnositelj);
    //obveznik u strAset = prioritet 2
    
    //
    strA.setBrojOsoba(p2.getBrojOsoba());
    strA.setBrojRedaka((long) p2.getBrojRedaka());
    //porez
    ...
    porez.setP6(p2.getSumPorPrirKam()); //
    ...
    zos.setP11(p2.getSumZdrSD());
    zos.setP12(p2.getSumZasSD());
    ...
    SNaknadaZaposljavanjeInvalida inv = factory.createSNaknadaZaposljavanjeInvalida();
    inv.setP1(p2.getBrojInvalida());
    inv.setP2(p2.getIznosInv());
    strA.setNaknadaZaposljavanjeInvalida(inv);
  ...
  private SStranaB getStranaB() throws Exception {
    ...
    for (p2.getStrBset().first(); p2.getStrBset().inBounds(); p2.getStrBset().next()) {
      ...
      p.setP100(strBset.getInt("NSATI"));
  ...
  private SJOPPDmetapodaci getMeta() throws Exception {
    SJOPPDmetapodaci meta = MetaGenerator.generateSJOPPDmeta(raUser.getInstance().getImeUsera(), System.currentTimeMillis()+"", 
        "Izvješće o primicima, porezu na dohodak i prirezu te doprinosima za obvezna osiguranja", "ObrazacJOPPD-v1-1");
    return meta;
  }
